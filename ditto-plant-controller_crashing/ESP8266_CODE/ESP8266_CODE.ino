...
const char* userId = "6748f0c3783e613c5e037339";
...
DHT dht(airHumTempPin, DHTTYPE, 22);
...
void updateServerAndRelaysState() {
    ...
    const char *first = "{\"operationName\": \"UpdatePlant\",\"variables\":{\"id\": \"6748f0c3783e613c5e037339\", \"contrId\": 3, \"hum1\": ";
...
void readTempeture() {
    ...
    tempeture = static_cast<int>(air_temp);
}

void readAirHumidity() {
    float humReading = dht.readHumidity();
    airHumidity = static_cast<int>(humReading);
}
...
void wifiSetupStuff() {
   ...
   // ESP.restart();  
    ...
    res = wm.autoConnect("DittoBot ID:4"); // password protected ap

    if(!res) { 
      // try once more with a new wifi manager
      Serial.println(F("Failed to connect, Trying wifi manager again"));
      WiFiManag`r wifiManager2;
      wifiManager2.setConfigPortalTimeout(100);  // seconds
      wifiManager2.setDebugOutput(true);
      wifiManager2.autoConnect("DittoBot ID:4");
    } else { 
